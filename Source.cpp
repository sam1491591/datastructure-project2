...
void map::read_floor(int m, int n) {
	char in;
	for (int i = 0; i <= m; i++)
	{
		for (int j = 0; j <= n; j++)
...
void map::distancecheck(tilt a,int m,int n)
{
	...
	while (!q.empty())
	{
		...
		//cout << s.x << " " << s.y << endl;
		q.pop();
		if (visited[s.x][s.y - 1] == 0 && floor1[s.x][s.y - 1] == 0 && s.y > 0)
		{
		...
		if (visited[s.x][s.y + 1] == 0 && floor1[s.x][s.y + 1] == 0 && s.y < n - 1)
		{
		...
		if (visited[s.x - 1][s.y] == 0 && floor1[s.x - 1][s.y] == 0 && s.x > 0)
		{
		...
		if (visited[s.x + 1][s.y] == 0 && floor1[s.x + 1][s.y] == 0 && s.x < m - 1)
		{
